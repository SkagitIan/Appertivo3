...
class GetConcepts:
    ...
    def generate_batch(self):
        ...
        for c in concepts:
            try:
                ...
                # --- Generate dish images concurrently ---
                images = {}
                with ThreadPoolExecutor(max_workers=3) as executor:
                    ...
                    for future in as_completed(futures):
                        dish = futures[future]
                        try:
                            images[dish["title"]] = future.result()
                        except Exception as exc:
                ...
                results.append({
                    ...
                    "dishes": [
                        {**d, "image_url": images.get(d["title"], self.DEFAULT_DISH_IMAGE_URL)}
                        for d in dishes
                    ],
                })
    ...
    def _generate_dishes_for_concept(self, concept):
        ...
        response = self.openai_client.responses.create(
            model="gpt-4.1-mini",
            input=self.dish_prompt(concept),
    ...
    def _generate_image(self, dish) -> str:
        ...
        image_prompt = f"""
                Create a high-definition, photorealistic food image of {dish}

                Focus tightly on the dish itself, styled on a dark wood or slate surface that matches a classic Pacific Northwest steakhouse.
